...
from app.db import get_session, Account, Contact, Signal, IntentScore

router = APIRouter(prefix="/accounts", tags=["accounts"])


@router.get("")
def list_accounts(strategy_id: str, db: Session = Depends(get_session)) -> list[dict]:
    ...
    for a in accounts:
        ...
        intent = db.query(IntentScore).filter(IntentScore.account_id == a.id).order_by(IntentScore.computed_at.desc()).first()
        out.append({
            "id": a.id,
...
def prioritized(strategy_id: str, db: Session = Depends(get_session)) -> dict:
    rows = list_accounts(strategy_id, db)
    return {
