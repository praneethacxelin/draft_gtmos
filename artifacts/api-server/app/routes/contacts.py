...
from app.db import get_session, Contact, Account

router = APIRouter(prefix="/contacts", tags=["contacts"])


@router.get("")
def list_contacts(strategy_id: str, tier: int | None = None, db: Session = Depends(get_session)) -> list[dict]:
    q = db.query(Contact).filter(Contact.strategy_id == strategy_id)
    if tier:
...
def get_contact(contact_id: str, db: Session = Depends(get_session)) -> dict:
